...
func TestCollectDependenciesNoDuplicates(t *testing.T) {
	depsFileWithDuplicates := NPMShrinkwrap{
		Name: "my-app",
		Version: "0.0.4",
		Dependencies: map[string]*NPMShrinkwrapDependency{
			"bluebird": &NPMShrinkwrapDependency{
	...
	deps := CollectDependencies(depsFileWithDuplicates)
	if len(deps) != 3 {
		t.Errorf("Expected to find three dependencies")
	}
